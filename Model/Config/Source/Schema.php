...
namespace Monogo\TypesenseCatalogCategories\Model\Config\Source;
...
class Schema extends AbstractTable
{
     ...
     * @var array
     */
    protected array $categoryAttributes = [];
    ...
    {
        $tableSchema = [];
        $fields = $this->connection->getConnection()->describeTable('catalog_category_entity');
        foreach ($fields as $field) {
            $tableSchema[$field['COLUMN_NAME']] = $field['COLUMN_NAME'] . ' (' . $field['DATA_TYPE'] . ')';
        }
        unset($tableSchema['entity_id']);

        return array_merge($this->getCategoryAttributes(), $tableSchema);
    }
     ...
     * @throws LocalizedException
     */
    protected function getCategoryAttributes(): array
    {
        ...
        foreach ($categoryAttributes as $attributeCode) {
            ...
            $this->categoryAttributes[$attributeCode] =
                $attributeCode . ' (' .
                str_replace('catalog_category_entity_', '', $attribute->getBackendTable()) . ')';
        }
